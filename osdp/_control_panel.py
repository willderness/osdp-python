...
class ControlPanel:
    ...
    def reader_led_control(
        ...
        connection_id: UUID,
        address: int,
        reader_led_controls: ReaderLedControls,
        should_wait: bool = True,
    ) -> bool:
        reply = self.send_command(
            connection_id,
            ReaderLedControlCommand(address, reader_led_controls),
            should_wait,
        )
        return should_wait == False or reply.type == ReplyType.Ack
    ...
    def send_command(
        self, connection_id: UUID, command: Command, should_wait: bool = True
    ) -> Reply:
        event = DataEvent()
